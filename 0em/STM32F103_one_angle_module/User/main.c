...
int main(void)
{
	...
	usart2_init(19200); 							//初始化串口2,用于与指纹模块通讯 ，里面会初始化一个定时器tim4 作为接收数据 的判断 
	KeyGPIOInit();
	LEDGPIOInit();
	LockGPIOInit();
	Moto_ZeroGPIOInit();
	...
	San_PWM_Init(PWM_PERIOD, PWM_PRESCALER);		// tim3 200hz   PB0 PB1  电机
	ws2812bInit();									//里面会开一个定时器tim1作为pwm输出 
	
    DebugGPIOInit();
	BeepInit(0xff,8);                             //蜂鸣器频率定时器初始化  (72M /(8 + 1) = 8M)
	IWDG_Init(4, 3750);								//分频系数为64，重载值为625，溢出时间为1S                      3750    6S
	SysInit();
	GENERAL_TIMx_Configuration();                   //tim2 tick

	//BeepSound(400, 500);   //debug 步进电机 
...
void SysTickTask(void)
{
    ...
    if(SysTask.u8Usart2RevTime){
        SysTask.u8Usart2RevTime--;
        if(SysTask.u8Usart2RevTime == 0){
            USART2_RX_STA		|= 1 << 15; 			//标记接收完成
        }
    }

    if(SysTask.u16BeepWaitTime){
        
		GPIO_SetBits(GPIOC, GPIO_Pin_13);
        SysTask.u16BeepWaitTime--;
    }
    else
